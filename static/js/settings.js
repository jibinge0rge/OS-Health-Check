...
async function renderVendorTab() {
  ...
  wrap.querySelectorAll("[data-source-card]").forEach((card) => {
    ...
    addBtn?.addEventListener("click", async () => {
      const value = prompt("Add family keyword");
...
async function renderAiTab() {
  ...
  const providers = [
    { id: "openai", label: "OpenAI", model: "gpt-4o-mini", available: settings.openai_available },
    { id: "gemini", label: "Gemini", model: "gemini-2.0-flash", available: settings.gemini_available },
    { id: "openrouter", label: "OpenRouter", model: "no key", available: settings.openrouter_available },
  ];
  document.getElementById("ai-provider-chips").innerHTML = providers
    .map((p) => `<span class="provider-chip ${p.available ? "" : "unavailable"} ${p.id === settings.ai_provider ? "active" : ""}" data-provider="${p.id}"><strong>${p.label}</strong><span>${p.model}</span></span>`)
    .join("");
