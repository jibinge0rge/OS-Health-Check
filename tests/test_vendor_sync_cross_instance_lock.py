...
class RaiseIfVendorSyncRunningTests(unittest.TestCase):
    def test_file_mode_never_checks_the_db(self) -> None:
        with (
            patch.object(app, "_USE_DB", False),
            patch.object(app.lookup_db, "db_sync_lock_status") as status_mock,
        ):
            app.raise_if_vendor_sync_running()
        status_mock.assert_not_called()

    def test_db_mode_no_lock_held_does_not_raise(self) -> None:
        with (
            patch.object(app, "_USE_DB", True),
            patch.object(app.lookup_db, "db_sync_lock_status", return_value=None),
        ):
            app.raise_if_vendor_sync_running()  # must not raise

    def test_db_mode_lock_held_raises_409_naming_the_blocker(self) -> None:
        with (
            patch.object(app, "_USE_DB", True),
            patch.object(
                app.lookup_db,
...
class RefreshEndpointsBlockedDuringVendorSyncTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_single_row_refresh_blocked_and_never_runs_the_lookup(self) -> None:
        p1, p2 = self._sync_running()
        with p1, p2, patch.object(app, "refresh_rows_lifecycle_chunk") as chunk_mock:
            with self.assertRaises(app.HTTPException) as ctx:
...
class VendorSyncStartBlockedByAnotherHolderTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_eosl_sync_blocked_and_never_runs_the_scrape(self) -> None:
        p1, p2, p3 = self._already_held()
        with p1, p2, p3, patch.object(app, "eosl_sync_os_database") as scrape_mock:
            with self.assertRaises(app.HTTPException) as ctx:
                await app.eosl_sync()
        self.assertEqual(ctx.exception.status_code, 409)
    ...
    async def test_junos_sync_blocked_and_never_runs_the_scrape(self) -> None:
        p1, p2, p3 = self._already_held()
        with p1, p2, p3, patch.object(app, "sync_junos_database") as scrape_mock:
            with self.assertRaises(app.HTTPException) as ctx:
    ...
    async def test_suse_sync_blocked_and_never_runs_the_scrape(self) -> None:
        p1, p2, p3 = self._already_held()
        with p1, p2, p3, patch.object(app, "sync_suse_database") as scrape_mock:
            with self.assertRaises(app.HTTPException) as ctx:
                await app.suse_sync()
        self.assertEqual(ctx.exception.status_code, 409)
        scrape_mock.assert_not_called()
    ...
    async def test_generic_vendor_sync_blocked_and_never_runs_the_scrape(self) -> None:
        p1, p2, p3 = self._already_held()
        with p1, p2, p3, patch.object(app, "vendor_sync_source") as scrape_mock:
            with self.assertRaises(app.HTTPException) as ctx:
    ...
    async def test_file_mode_never_touches_the_db_lock(self) -> None:
        with (
            patch.object(app, "_USE_DB", False),
            patch.object(app.lookup_db, "db_acquire_sync_lock") as acquire_mock,
            patch.object(app, "eosl_sync_os_database", return_value={}),
